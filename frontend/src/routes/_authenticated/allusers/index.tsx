...
import { CreateUserDialog, columns } from '@/features/allusers';
...
function RouteComponent() {
	...
	async function handleBulkRemove() {
		...
		await deleteUser.mutateAsync({ id }, {
			onSuccess: async () => {
				...
				showInfoToast(`(${id.length}) کاربر با موفقیت حذف شد`);
				setRowSelection({});
			},
		});
	}
	...
	const query = useQuery({
		...trpc.users.listAll.queryOptions({
			q: search.q,
			page: search.page,
			limit: search.limit,
		}),
			...
			<DataTableToolbar>
				...
				<div className="flex items-center gap-2">
					...
					<BulkDeleteButton
						...
						title="حذف کاربران ؟"
						description="این عملیات برای همیشه این کاربران را حذف می‌کند"
					/>
				</div>
			</DataTableToolbar>
			<DataTable
