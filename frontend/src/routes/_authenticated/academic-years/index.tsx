...
import { CreateAcademicYearDialog, columns } from "@/features/academic-years";
...
function RouteComponent() {
	...
	const deleteAcademicYear = useMutation(
		trpc.academicYears.delete.mutationOptions({
			onError: (error) => {
		...
		await deleteAcademicYear.mutateAsync(
			{ id: ids },
			{
				onSuccess: async () => {
					...
					showInfoToast(`(${ids.length}) سال تحصیلی با موفقیت حذف شد`);
					setRowSelection({});
				},
			}
		);
		...
		<div className="flex flex-col gap-4">
			<DataTableToolbar>
				<div className="flex flex-1 items-center gap-2">
					<DataTableSearch
						placeholder="جستجو سال‌های تحصیلی..."
						value={search.q}
				...
				<div className="flex items-center gap-2">
					...
					<BulkDeleteButton
						...
						title="حذف سال‌های تحصیلی ؟"
						description="این عملیات سال‌های تحصیلی انتخاب شده را حذف می‌کند."
					/>
				</div>
			</DataTableToolbar>
			<DataTable
